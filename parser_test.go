...
const testStringRegex = `Domain Name: WIKIPEDIA.ORG
...
func TestParserRegexSimple(t *testing.T) {
	val := regex(`(?sU)Registrar:([^\r\n]*)?[\n\r]+`, testStringRegex)
	if len(val) < 1 {
		t.Errorf("regex should return an array")
	} else if len(val) > 1 {
		t.Errorf("regex should return only one result")
	} else if val[0] == "" {
		t.Errorf("regex should not be empty")
	}
}

func TestParserRegexMulti(t *testing.T) {
	val := regex(`(?sU)Status:[\s\t]+([^\r\n]*)?\s+`, testStringRegex)
	if len(val) < 1 {
		t.Errorf("regex should return an array")
	} else if len(val) < 2 {
		t.Errorf("regex should return 2 results")
	}
}

func TestParserRegexNull(t *testing.T) {
	val := regex(`(?sU)Whois Server:([^\r\n]*)?[\n\r]+`, testStringRegex)
	if len(val) < 1 {
		t.Errorf("regex should return an array")
	} else if len(val) > 1 {
		t.Errorf("regex should return only one result")
	} else if val[0] != "" {
		t.Errorf("regex should be empty")
	}
}
